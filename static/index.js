$(function () {
  ...
  socket.on('add username', (data) => {
    if (data['error'] != '') {
      ...
      $('#username').text(
        'Welcome to Flack App ' + localStorage.getItem('username')
      );
      $('#General').click();
      $('#messageInput').focus();
    }
  ...
  $('#username').text(
    'Welcome to my Flack Chat App ' + localStorage.getItem('username')
  );
});
...
function loadMessages(data) {
  ...
  for (x in data['channels'][activeChannel]) {
    ...
    if (
      data['channels'][activeChannel][x]['username'] ==
...
function loadPrivateMessages(data, otherUser) {
  $('#messages').html('');
  for (message in data['privateMessages'][localStorage.getItem('username')][
    otherUser
  ]) {
    const media = document.createElement('div');
    if (
      data['privateMessages'][localStorage.getItem('username')][otherUser][
        message
